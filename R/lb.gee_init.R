#' Starting Values Searching Function
#'
#' A function to help log binomial model finding the admissible starting values.
#' @param formula The formula of model.
#' @param data The data to run the regression.
#' @param noconstant No constant anticipated in the regression processes, Defaults to FALSE.
#'
#' starting_value()

lb.gee.init<-function(x,y, noconstant=FALSE)
{
  for(u in 1:3)
  {
    if(u==1) {
    ...
    if(all(mu_old<1 & mu_old>0)) {
      ...
      if(noconstant==FALSE) {
        conv<-1
        init_mu_old$coefficients[1]<-init_mu_old$coefficients[1]-max(init_mu_old$fitted.values)-1
